...
class HashMap {
  ...
  set(key, value) {
    ...
    if (loadRatio > HashMap.MAX_LOAD_RATIO) {
      // MAX_LOAD_RATIO = keeps track of how full the hashmap is. When it is a certain % full, we move to a bigger hash table using the SIZE_RATIO so we reduce the number of collisions. Helps to improve the performance of value retrieval
  ...
  _resize(size) {
    ...
    this._capacity = size;
    // Reset the length - it will get rebuilt as you add the items abck
    this.length = 0;
    this._deleted = 0;
    this._hashTable = [];
